...
class WordSearch {
    ...
    private func labels(fromX x: Int, y: Int, word: String, movement: (x: Int, y: Int)) -> [Label]? {
        ...
        for letter in word {
            let lable = labels[xPosition][yPosotion]
            if lable.letter == " " || lable.letter == letter {
                returnValue.append(lable)
                xPosition += movement.x
                yPosotion += movement.y
            } else {
                return nil
            }
        }
    ...
    func render() -> Data {
        ...
        let gridLetterAttribute: [NSAttributedString.Key: Any] = [
            .font: gridLetterFont,
            .paragraphStyle: gridLetterStyle
        ]
        
        let render = UIGraphicsPDFRenderer(bounds: pageRect)
        return render.pdfData(actions: { (ctx) in
            for _ in 0 ..< numberOfPages {
                ...
                for column in labels {
                    for label in column {
                        let size = String(label.letter).size(withAttributes: gridLetterAttribute)
                        ...
                        String(label.letter).draw(in: cellRect, withAttributes: gridLetterAttribute)
